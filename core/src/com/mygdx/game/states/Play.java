...
public class Play extends GameState {
	...
	public Play(GameStateManager m) {
		...
		// HUD---------------
		backButton = new SimpleImageButton(Game.cm.getTexture("backButton"), 30, Game.VHEIGHT - 150 - 20, 150, 150,
				false, true);
		...
		Gdx.input.setInputProcessor(new InputMultiplexer(new InputProcessor() {
			...
			public boolean keyUp(int keycode) {
				if (keycode == Keys.ESCAPE) {
					getStateManager().setTransition(GameStateManager.RIGHTLEFT, Play.this, GameStateManager.LEVELSELECT,
							true, false);
					return true;
				}
	...
	public void update(float dt) {
		...
		if (lcm.continueIsClicked()) {
			getStateManager().setTransition(GameStateManager.RIGHTLEFT, Play.this, GameStateManager.LEVELSELECT, true,
					false);
		}
	...
	public void render() {
		...
		font.draw(s, layout, hudBoxes.getX() + hudBoxes.getWidth() + 5, hudBoxes.getY());

		s.end();

		distanceIndicator.render(s);
	...
	public void finishStage(boolean finished, int cratesOut) {
		...
		// HIDE BUTTONS
		Tween.to(backButton.getSprite(), TweenSpriteAccessor.POS_XY, 0.5f).targetRelative(0, +150 + 20)
